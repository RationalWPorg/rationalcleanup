...
if ( ! function_exists( 'rationalwp_render_parent_page' ) ) {
	...
	function rationalwp_render_parent_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'RationalWP Plugins', 'rationalcleanup' ); ?></h1>
			<p><?php esc_html_e( 'Practical tools for WordPress professionals.', 'rationalcleanup' ); ?></p>

			<?php if ( empty( $plugins ) ) : ?>
				<div class="notice notice-warning">
					<p>
						<?php esc_html_e( 'Unable to load plugin information. Please check your internet connection and try again later.', 'rationalcleanup' ); ?>
						<?php if ( current_user_can( 'manage_options' ) ) : ?>
							<br>
							<?php /* translators: %s: URL where plugin data is fetched from */ ?>
							<small><?php printf( esc_html__( 'Plugin data is fetched from %s', 'rationalcleanup' ), '<code>' . esc_html( RATIONALWP_PLUGINS_JSON_URL ) . '</code>' ); ?></small>
						<?php endif; ?>
					</p>
				</div>
			<?php else : ?>
				...
				<?php foreach ( $plugins as $plugin_key => $plugin ) : ?>
					...
					<div class="rationalwp-plugin-card" style="background: #fff; border: 1px solid #c3c4c7; border-radius: 4px; padding: 20px;">
						<h3 style="margin-top: 0; margin-bottom: 10px;">
							...
							<?php if ( $is_active ) : ?>
								<span style="background: #00a32a; color: #fff; font-size: 11px; padding: 2px 8px; border-radius: 3px; margin-left: 8px; vertical-align: middle;"><?php esc_html_e( 'Active', 'rationalcleanup' ); ?></span>
							<?php elseif ( $is_installed ) : ?>
								<span style="background: #dba617; color: #fff; font-size: 11px; padding: 2px 8px; border-radius: 3px; margin-left: 8px; vertical-align: middle;"><?php esc_html_e( 'Inactive', 'rationalcleanup' ); ?></span>
							<?php endif; ?>
						</h3>
						<p style="color: #646970; margin-bottom: 15px;"><?php echo esc_html( $plugin['description'] ); ?></p>
						<div class="rationalwp-plugin-actions">
							<?php if ( $is_active ) : ?>
								<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . $plugin['menu_slug'] ) ); ?>" class="button button-primary"><?php esc_html_e( 'Settings', 'rationalcleanup' ); ?></a>
							<?php elseif ( $is_installed ) : ?>
								<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'plugins.php?action=activate&plugin=' . urlencode( $plugin['file'] ) ), 'activate-plugin_' . $plugin['file'] ) ); ?>" class="button button-primary"><?php esc_html_e( 'Activate', 'rationalcleanup' ); ?></a>
							<?php else : ?>
								<a href="<?php echo esc_url( $plugin['url'] ); ?>" class="button" target="_blank"><?php esc_html_e( 'Learn More', 'rationalcleanup' ); ?></a>
							<?php endif; ?>
						</div>
					</div>
				<?php endforeach; ?>
